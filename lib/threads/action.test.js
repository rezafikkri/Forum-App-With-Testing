...
import api from '../api';
import { asyncDownVoteThread, asyncNeutralVoteThread, asyncUpVoteThread, downVoteThreadActionCreator, neutralVoteThreadActionCreator, upVoteThreadActionCreator } from './action';
 ...
 *   - should dispatch action correctly (without DOWN_VOTE_THREAD action), call api function correctly 
 *   and throw error when up-vote failed and isDownVoted = false
...
describe('asyncNeutralVoteThread thunk', () => {
  ...
  test('should dispatch action correctly when neutral-vote success', async () => {
    // Arrange
    api.neutralVoteThread = () => Promise.resolve(fakeNeutralVoteResponse);
    const dispatch = vi.fn();
  ...
  test('should dispatch action correctly (with UP_VOTE_THREAD action) and throw error when neutral-vote failed and target = \'up-vote\'', async () => {
    // Arrange
    api.neutralVoteThread = () => Promise.reject(fakeErrorResponse);
    const dispatch = vi.fn();
    const getState = () => ({ authUser: { id: fakeNeutralVoteResponse.userId } });
    // Action
  ...
  test('should dispatch action correctly (with DOWN_VOTE_THREAD action) and throw error when neutral-vote failed and target != \'up-vote\'', async () => {
    // Arrange
    api.neutralVoteThread = () => Promise.reject(fakeErrorResponse);
    const dispatch = vi.fn();
    const getState = () => ({ authUser: { id: fakeNeutralVoteResponse.userId } });
